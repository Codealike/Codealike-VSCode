    ...
    function () {
      ...
      vscode.window
        .showInputBox({
          ...
          ignoreFocusOut: true,
          placeHolder:
            "Enter here your Codealike API token, or clean it to disconnect",
...
function startTrackingProject() {
  ...
  Codealike.configure(vscode.workspace.workspaceFolders[0].uri.fsPath).then(
    (configuration) => {
        ...
        function (result) {
          ...
          if (result) {
            result.forEach(function (element) {
              if (!element || element.location.range._start.line > line) return;
